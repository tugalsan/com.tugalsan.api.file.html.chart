...
public class TS_FileHtmlChartUtils {
    ...
    public static void addHTML_chart(List<String> strBuffer, List<HTMLChart> charts, CharSequence googleChartsJs) {
        ...
        IntStream.range(0, charts.size()).forEachOrdered(i -> {
            ...
            var c = charts.get(i);
            addLine(destI, TGS_StringUtils.concat("        function getDataLineChart", String.valueOf(i), "(){"));
            addLine(destI, "            var data = new google.visualization.DataTable();");
            addLine(destI, "            data.addColumn('string', 'X');");
            addLine(destI, TGS_StringUtils.concat("            data.addColumn('number', '", c.seriNames.get(0), "');"));
            addLine(destI, TGS_StringUtils.concat("            data.addColumn('number', '", c.seriNames.get(1), "');"));
            ...
            IntStream.range(0, c.data.size()).forEachOrdered(di -> {
                ...
                nem = nem == 0f ? null : nem;
                sic = sic == 0f ? null : sic;
                addLine(destI, TGS_StringUtils.concat("                ['", dataI.xLabel, "', ", String.valueOf(nem), ", ", String.valueOf(sic), "]", (di != c.data.size() - 1 ? "," : "")));
            });
            addLine(destI, "            ]);");
            addLine(destI, "            return data;");
            addLine(destI, "        }");
            addLine(destI, TGS_StringUtils.concat("      function getOptionsLineChart", String.valueOf(i), "() {"));
            ...
            addLine(destI, "          hAxis: {");
            addLine(destI, TGS_StringUtils.concat("            title: '", c.xTitle, "',"));
            ...
            addLine(destI, "          },");
            addLine(destI, "          vAxis: {");
            addLine(destI, TGS_StringUtils.concat("            title: '", c.yTitle, "',"));
            addLine(destI, "            slantedText: true,");
            addLine(destI, "            textStyle: {");
            addLine(destI, "               fontSize: 10");
            addLine(destI, "            },");
        ...
        IntStream.range(0, charts.size()).forEachOrdered(i -> {
            List<String> destI = TGS_ListUtils.of();
            addLine(destI, "      <tr>");
            addLine(destI, TGS_StringUtils.concat("          <td id=\"divLineChart", String.valueOf(i), "\" style=\"width: 900px; height: 250px;\"></td>"));
            addLine(destI, "     </tr>");
            addLine(destI, "      <tr>");
